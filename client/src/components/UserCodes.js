      ...
      <div className='grid grid-cols-1 md:grid-cols-5 gap-2'>
        {codes.map((code) => <CodeCard key={code._id} title={code.title} code={code.code} date={code.date} id={code._id} children={<div className="flex justify-evenly">
          <Link to="/landing" state={{
            ...
            title: code.title,
            code: code.code
          }} className="inline-flex items-center px-3 py-2 text-xs lg:text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            Edit
          </Link>
          <button type="button" className="inline-flex items-center px-3 py-2 text-xs lg:text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" onClick={() => deleteCode(code._id)}>
            Delete
          </button>
        </div>} />)}
      </div>
